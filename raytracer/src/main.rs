...
fn cornell_box() {
    ...
    let picture = raytracer::canvas::Canvas::empty(image_width, image_height);
    let raytracer = RayTracer::new(camera, picture, world.build(), 50);
    raytracer.render().save("output/book3/image15.png");
}
...
fn final_scene(image_width: u32, sample_per_pixel: u32, max_depth: u32) {
    ...
    world.add_light(Quad::new(Vec3::new(123.0, 554.0, 147.0),
                              Vec3::new(300.0, 0.0, 0.0),
                              Vec3::new(0.0, 0.0, 265.0)));
    world.add_light(Sphere::new(Vec3::new(260.0, 150.0, 45.0), 50.0));
    // world.add_light(Sphere::new(Vec3::new(360.0, 150.0, 145.0), 70.0));
...
fn test_obj() {
    File::open("assets/earth_map.jpg").unwrap();
    File::open("assets/cornell_box.obj").unwrap();

    let cornell_box = tobj::load_obj("assets/cornell_box.obj", &tobj::GPU_LOAD_OPTIONS);

...
fn quads() {
    ...
    // ));
    world.add_object(Object::new(
        ShapeList::load_obj("assets/cornell_box.obj").tree(),
        TexturedMaterial::new(SolidColor::new(Color::new(0.2, 0.8, 0.8)), Lambertian),
    ));
    // world.add_object(create_lambertian(
    ...
    // ));
    world.set_background(Color::WHITE);

    let image_width = 400;
    let image_height = 400;
    let camera = Camera::new(
        PerspectiveParam {
            look_from: Vec3::new(0.0, 10.0, 10.0),
            look_at: Vec3::new(0.0, 0.0, 0.0),
            view_up: Vec3::new(0.0, 1.0, 0.0),
        },
        LensParam {
